...
class CalendarDialog(QDialog):
    ...
    def onDateChanged(self):
        ...
        selected_date_str = selected_date.toString(Qt.ISODate)  # 使用 ISO 标准格式
        s_t = time.strptime(selected_date_str, "%Y-%m-%d")  # 返回元祖
        mkt = int(time.mktime(s_t))
        timestamp = mkt
        self.selected_date_signal.emit(timestamp)
        print("Selected Date:", selected_date_str, timestamp)
        self.close()
